...
public class StockItemImportJob {
	...
	/**
	 * Parses an {@link ItemType} from a CSV cell value.
	 * Accepts numeric codes ({@code 0}, {@code 1}, {@code 2}) and descriptive names.
	 *
	 * @return the resolved {@link ItemType}, or {@code null} if the cell was blank
	 *         (blank means "derive from context").
	 * @throws IllegalArgumentException when the value is non-blank but unrecognisable.
	 */
	private ItemType parseItemType(String raw) {
		if (isBlank(raw)) {
			return null; // caller will derive from context
		}
		String token = raw.trim().toLowerCase().replace(" ", "_");
		switch (token) {
			case "0":
			case "non_pharmaceutical":
				return ItemType.NON_PHARMACEUTICAL;
			case "1":
			case "pharmaceutical":
			case "drug":
	...
	private Object validateLine(String[] line) {
		...
		// Increased to 22 to accommodate the new ITEM_TYPE column
		...
		// ----------------------------------------------------------------
		// ITEM_TYPE (column 21) – optional, validated when present.
		// A drug_id row must not declare itself as NON_PHARMACEUTICAL or
		// LAB_COMMODITY since those types don't have an associated Drug.
		// ----------------------------------------------------------------
		if (line.length > ITEM_TYPE && !isBlank(line[ITEM_TYPE])) {
			try {
				ItemType parsedType = parseItemType(line[ITEM_TYPE]);
				if (parsedType != null) {
					// A drug_id is only compatible with PHARMACEUTICAL
					if (objects[DRUG_ID] != null && parsedType != ItemType.PHARMACEUTICAL) {
						errors.add(Context.getMessageSourceService().getMessage(
								"stockmanagement.importoperation.drugidmustbepharmaceutical"));
					} else {
						objects[ITEM_TYPE] = parsedType;
					}
				}
			} catch (IllegalArgumentException ex) {
				errors.add(Context.getMessageSourceService().getMessage(
						"stockmanagement.importoperation.itemtypeinvalid"));
			}
		}
	...
	private ItemType resolveItemType(Object[] parsedRow) {
		if (parsedRow[ITEM_TYPE] != null) {
			return (ItemType) parsedRow[ITEM_TYPE];
		}
	...
	private void updateStockItems(Map<Integer, Object[]> stockItems) {
		...
		List<StockItemDTO> stockItemIds = stockManagementService.getExistingStockItemIds(stockItems.values().stream().map(p -> {
			...
			if (p[DRUG_ID] != null) {
				...
				// Drug rows are always PHARMACEUTICAL
				itemGroupFilter.setItemType(ItemType.PHARMACEUTICAL);
			} else {
				itemGroupFilter.setConceptId((Integer) p[CONCEPT_ID]);
				// Use the resolved type (NON_PHARMACEUTICAL or LAB_COMMODITY)
				itemGroupFilter.setItemType(resolveItemType(p));
			}
		...
		for (Map.Entry<Integer, Object[]> recordToProcess : stockItems.entrySet()) {
			...
			// Tracks whether a brand-new record was created for a drug (true),
			// a concept-based item (false), or this is an update (null).
			...
			if (isNewRecord) {
				...
				if (updates[DRUG_ID] != null) {
					// -------------------------------------------------------
					// Drug-based item → always PHARMACEUTICAL
					// -------------------------------------------------------
					...
					if (stockItem != null) {
						...
						// Use setItemType instead of the deprecated setIsDrug
					...
					if (stockItem != null) {
						...
						// Resolve the correct type: NON_PHARMACEUTICAL or LAB_COMMODITY
						stockItem.setItemType(resolveItemType(updates));
					}
				}
			...
			// ---------------------------------------------------------------
			// If an explicit item_type was supplied on an UPDATE row, apply it
			// (guards against reclassifying a drug item by accident)
			// ---------------------------------------------------------------
			if (!isNewRecord && updates[ITEM_TYPE] != null) {
				ItemType requestedType = (ItemType) updates[ITEM_TYPE];
				// Never reclassify a drug-linked item away from PHARMACEUTICAL
				if (stockItem.getDrug() != null && requestedType != ItemType.PHARMACEUTICAL) {
					result.getErrors().add(String.format("Row %1s: %2s", recordToProcess.getKey(),
					    Context.getMessageSourceService().getMessage("stockmanagement.importoperation.drugidmustbepharmaceutical")));
					continue;
				}
				if (stockItem.getItemType() != requestedType) {
					stockItem.setItemType(requestedType);
				}
			}
